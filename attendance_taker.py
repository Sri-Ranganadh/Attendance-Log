...
class Face_Recognizer:
    ...
    def attendance(self, StudentID, current_time, in_time=True):
        ...
        AttendanceID = f"{StudentID}_{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}"

        # Query to select the latest entry for the StudentID
        query = "SELECT * FROM Attendance WHERE StudentID = ? ORDER BY Date DESC, InTime DESC LIMIT 1"
                        ...
                        for k in range(len(faces)):
                            ...
                            if min(self.current_frame_face_X_e_distance_list) < 0.4:
                                ...
                                if datetime.datetime.now().strftime('%p') == 'AM':
                                    ...
                                    shift = 'Afternoon'
                                print(name)
                                self.attendance(name, current_time, in_time=True)
                            else:
